...
tresult PLUGIN_API DiaProController::initialize (FUnknown* context)
{
    ...
    // compressor
    unitInfo.id = DiaProCompressorUnitId;
    ...
    // de-esser
    unitInfo.id = DiaProDeEsserUnitId;
    unitInfo.parentUnitId = kRootUnitId; // attached to the root unit
    ...
    // exciter
    unitInfo.id = DiaProExciterUnitId;
    unitInfo.parentUnitId = kRootUnitId; // attached to the root unit
    ...
    // output
    unitInfo.id = DiaProOutputUnitId;
    unitInfo.parentUnitId = kRootUnitId; // attached to the root unit
    ...
    param->setUnitID(DiaProRootUnitId);
    ...
    param->setUnitID(DiaProCompressorUnitId);
    ...
    param->setPrecision(2);
    param->setUnitID(DiaProDeEsserUnitId);
    parameters.addParameter(param);
    ...
    param->setPrecision(2);
    param->setUnitID(DiaProExciterUnitId);
    parameters.addParameter(param);
    ...
    param->setPrecision(2);
    param->setUnitID(DiaProOutputUnitId);
    parameters.addParameter(param);
...
static const Steinberg::Vst::ParamID midi_cc_map[] = {
    [ControllerNumbers::kCtrlVolume] = 1 + kGainId,
    [ControllerNumbers::kCtrlGPC2] = 1 + kCompThreshId,
    [ControllerNumbers::kCtrlAttackTime] = 1 + kCompAttimeId,
    [ControllerNumbers::kCtrlReleaseTime] = 1 + kCompReltimeId,
    [ControllerNumbers::kCtrlGPC3] = 1 + kCompRatioId,
    [ControllerNumbers::kCtrlGPC4] = 1 + kCompKneeId,
    [ControllerNumbers::kCtrlGPC5] = 1 + kCompMakeupId,
    [ControllerNumbers::kCtrlPan] = 1 + kCompMixId,
    [ControllerNumbers::kCtrlGPC6] = 1 + kCompLookAheadId,
#if 0
    [-1] = 1 + kCompStereoLinkId,
    [-1] = 1 + kCompEnabledId, // Toggles don't really work with the current VST3 SDK version
#endif
    [ControllerNumbers::kCtrlGPC1] = 1 + kDeEsserThreshId,
    [ControllerNumbers::kCtrlEffect1] = 1 + kDeEsserFreqId,
    [ControllerNumbers::kCtrlEffect2] = 1 + kDeEsserDriveId,
#if 0
    [-1] = 1 + kDeEsserEnabledId,
#endif
    [ControllerNumbers::kCtrlExpression] = 1 + kExciterDriveId,
    [ControllerNumbers::kCtrlFilterCutoff] = 1 + kExciterFcId,
    [ControllerNumbers::kCtrlGPC7] = 1 + kExciterSatId,
    [ControllerNumbers::kCtrlBalance] = 1 + kExciterBlendId,
#if 0
    [-1] = 1 + kExciterEnabledId,
#endif
};
